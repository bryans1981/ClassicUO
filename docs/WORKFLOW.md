...
- Keep all work pointed at the live browser application milestone first. If a browser-related cleanup item is not a blocker, record it for later and move on.
...
- 2026-04-15: Added a browser-native websocket session controller on top of the browser runtime execution controller. This is the current browser-session handshake baseline and the first explicit websocket handshake layer.
 - 2026-04-15: Added a browser-native websocket runtime session controller on top of the browser websocket execution controller. This is the current browser-session handoff layer.
 - 2026-04-15: Added a browser-native browser session controller on top of the browser runtime execution controller. This remains the current browser-session baseline.
 - 2026-04-15: Added a browser-native websocket runtime execution controller on top of the websocket session controller. This is the current browser-runtime-execution handshake baseline and the first live browser websocket connect/read.
...
- 2026-04-20: The no-click self-test runner now opens Edge minimized by default so repeated validation does not clutter the desktop.
- 2026-04-20: The no-click self-test runner now uses Edge's `--start-minimized` launch flag plus minimized window style to keep browser windows out of the way during automated validation.
...
- 2026-04-21: Browser websocket receive now hands teardown back to `Disconnect()` when the loop exits, so disconnect/reconnect state and socket cleanup stay in one place.
...
- 2026-04-22: Browser startup validation failures no longer try to launch an external browser; the desktop-only launch path remains desktop-only.
- 2026-04-22: The desktop startup environment-variable setup is now isolated from the browser bootstrap path in a helper, so browser startup reads more like one linear flow.
...
- 2026-04-22: Desktop UO-directory and client-version validation now live in a helper that returns zero in browser mode, so the browser path stays linear in `Main`.
- 2026-04-22: The UO-directory fallback in `Main` now chooses the browser assets root directly when browser mode is active, instead of using a browser-specific branch.
...
- 2026-04-21: Browser startup now skips OS language probing in browser mode and defaults to `ENU` when the language is unset, so the browser client stays off desktop locale detection.
