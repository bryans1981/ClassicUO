...
public sealed class BrowserClilocReaderService
{
    ...
    public async ValueTask<BrowserClilocReadResult> ReadAsync(string path)
    {
        ...
        try
        {
            BrowserAssetLoadResult<ClilocFirstEntryParse> loadResult = await _loaderHarness.LoadParsedAsync(
                path,
                static bytes => bytes.Length >= 4 && bytes[3] == 0x8E ? BwtDecompressShim.Decompress(bytes) : bytes,
                static bytes => ParseCliloc(bytes),
            ...
            ClilocFirstEntryParse parsed = loadResult.Value;
    ...
    private static ClilocFirstEntryParse ParseCliloc(byte[] bytes)
    {
        ...
        return new ClilocFirstEntryParse(header1, header2, firstEntryNumber, firstEntryFlag, firstEntryTextLength, firstEntryText);
    }

    private readonly record struct ClilocFirstEntryParse(int Header1, short Header2, int FirstEntryNumber, byte FirstEntryFlag, short FirstEntryTextLength, string FirstEntryText);
}
