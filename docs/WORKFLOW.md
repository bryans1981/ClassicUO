...
- 2026-04-21: Browser startup now normalizes desktop gameplay toggles like save-account, auto-login, reconnect, reconnect timing, login music, maps layouts, the saved server selection, the saved files override, saved credentials, screen scale, and encryption, and the parser now ignores direct username/password and screen-scale overrides in browser mode.
